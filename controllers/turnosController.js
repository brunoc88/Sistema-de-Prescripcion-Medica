...
exports.turnosEmpleado = async (req, res) => {
    try {
        ...
        const turnos = await Turno.findAll({
            where: { id_profesional: profesional.idProfesional, fecha: hoy },
            include: [
                {
                    model: Paciente,
                    include: [{
...
exports.vistaPacientes = async (req, res) => {
    try {
        //obtengo los datos del medico logueado
        const medicoLogueado = req.user;
        //busco el medico mediante email
        const medico = await Profesional.findOne({ where: { email: medicoLogueado.email } });
        //busco todos los turnos relacionados al medico
        const turnos = await Turno.findAll(
            { include: [{ model: Profesional },{model:Paciente, include:{model:Plan, include:{model:ObrasSociales}}}] },
            { where: { id_profesional: medico.idProfesional } });
        //res.json(turnos)
        return res.status(200).render('empleado/pacientes',{pacientes:turnos});
    } catch (error) {
        return res.status(500).json('Hubo un error ' + error.message);
    }
}